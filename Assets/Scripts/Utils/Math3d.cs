...
public class Math3d
{
    ...
    // Returns random point inside triangle of given A, B, C
    public static Vector3 GetRandomPointInsideTriangle(Vector3 A, Vector3 B, Vector3 C)
    {
        float a = UnityEngine.Random.Range(0f, 1f);
        float b = UnityEngine.Random.Range(0f, 1f - a);
        float c = 0;
        ...
        px = (a * A.x) + (b * B.x) + (c * C.x);
        py = (a * A.y) + (b * B.y) + (c * C.y);
        pz = (a * A.z) + (b * B.z) + (c * C.z);
        return new Vector3(px, py, pz);
    }

    // Returns area's size of given polygon (vertices list)
    public static float GetPolygonAreaSize(List<Vector2> vertices)
    {
        float areaSize = 0f;
        for (int i = 0; i < vertices.Count; i++)
        {
            areaSize += vertices[i].x * vertices[(i + 1) % vertices.Count].y - vertices[(i + 1) % vertices.Count].x * vertices[i].y;
        }
        areaSize = Mathf.Abs(areaSize / 2f);
        return areaSize;
    }
}